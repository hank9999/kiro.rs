        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-4">
            <div className="rounded-lg border bg-muted/30 p-3">
              ...
              <div className="mt-1 text-2xl font-semibold">{summary?.totalRequests ?? 0}</div>
            </div>
              ...
              records.map((record) => (
                ...
                >
                  <div className="flex items-start gap-2">
                    <Badge variant={getStatusVariant(record.success, record.statusCode)}>
                      {record.statusCode}
                    </Badge>
            ...
            <div className="h-[28rem] overflow-auto rounded-lg border bg-muted/20 p-3 font-mono text-xs leading-5">
              {(logs?.lines.length ?? 0) === 0 ? (
                ...
                logs?.lines.map((line, index) => (
                  <div key={`${index}-${line.slice(0, 32)}`} className={getLogLineClass(line)}>
                    {line}
                  </div>
                ))
              )}
